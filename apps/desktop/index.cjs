...
function getIconPath() {
  if (electron.app.isPackaged) {
    return path.resolve(process.resourcesPath, "icon.ico");
  }
...
function createTray() {
  const trayIconPath = electron.app.isPackaged ? path.resolve(process.resourcesPath, "tray.png") : path.resolve(__dirname, "../../resources/tray.png");
...
async function checkForUpdates() {
  try {
    ...
    if (version && version !== electron.app.getVersion()) {
...
electron.app.whenReady().then(() => {
  createWindow();
  setTimeout(() => checkForUpdates(), 8e3);
